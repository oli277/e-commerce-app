...
export default function Categories() {
  ...
  const { data: Pdata, isloading: Pisloading, error: Perror } = useFetchProducts(urlProducts)
  ...
  const handleCategoryClick = (category) => {
    ...
    const filteredProducts = Pdata.filter(product => product.category === selectedCategory)

  if(selectedCategory) {
    if(filteredProducts.length < 1) {
      return(
        <div className="max-w-7xl mx-auto px-4 py-8">
      <h2 className="text-2xl font-bold text-center text-gray-800 mb-8"> {selectedCategoryTitle}</h2>
      <h3 className="text-2xl font-bold text-center text-gray-800 mb-8"> No products in this category </h3>
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          >
            ...
            <div className="relative w-full aspect-square bg-[#f8f8f8] rounded-md mb-4 overflow-hidden flex items-center justify-center">
              ...
              <div className="absolute top-2 left-0 right-0 flex justify-between px-2 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                ...
                >
                  <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-5 h-5">
                    <path strokeLinecap="round" strokeLinejoin="round" d="M7.5 3.75H6A2.25 2.25 0 0 0 3.75 6v1.5m13.5-3.75H18A2.25 2.25 0 0 1 20.25 6v1.5m-16.5 9V18A2.25 2.25 0 0 0 6 20.25h1.5m9-16.5v1.5c0 1.243 1.007 2.25 2.25 2.25h1.5m-12 9v1.5c0 1.243-1.007 2.25-2.25 2.25h-1.5m15 0V18a2.25 2.25 0 0 0-2.25-2.25h-1.5m-6 2.25 1.5-1.5" />
                  </svg>
                </button>
              </div>
            </div>
            ...
            <div className="flex flex-col flex-grow">
              ...
              <h3 className="text-sm text-gray-800 font-medium mb-3 line-clamp-1" title={product.name}>
                {product.name}
              </h3>
      ...
      <div className="bg-white py-6 flex justify-center">
        <button 
          onClick={() => {setselectedCategory("")}}
          className="cursor-pointer bg-[#ffc107] hover:bg-yellow-500 text-black px-12 py-3 rounded-md text-lg font-medium transition-colors"
        >
          Return to categories
        </button>
      </div>
